		...
		<?php			
			if(isset($_REQUEST['book_now']))
			  ...
			  {  
				...
				//----------------------------------------------------------------------------------------------------7-4-2022
				$sss="select * from room_over_charge_rate";
				$qqq=mysql_query($sss) or die(mysql_error());
				$rrr=mysql_fetch_assoc($qqq);
					$charge_amount=$overtime_less_5mins * $rrr['over_rate']; //OMAR CHANGE FROM 1.25 to 1.66666666667  7-4-2022
				
					$level=$_REQUEST['level'];
				    $id=$_REQUEST['id'];
				    $room_no=$_REQUEST['room_no'];
				    $charge_name="$ot_less_5mins mins or $ot_hour Hrs Overtime Charge(".$rrr['over_rate']."/min) after 5mins of time due";
				    $username=$_SESSION['username'];
  ...
  <?php if(isset($_REQUEST['settings'])) { ?>   
	<div class="w3-col">
     ...
     <!--settings menu-->
	  ...
	  <a href="admin.php?settings=1&roomcharge=1&level=<?php echo $_REQUEST['level'];?>" class="w3-padding"><i class="fa fa-money fa-fw"></i> Update Room Charge</a>
	  <a href="admin.php?settings=1&roomovercharge=1&level=<?php echo $_REQUEST['level'];?>" class="w3-padding"><i class="fa fa-money fa-fw"></i> Update Room Over Charge</a>
	  ...
	  if($_REQUEST['level']==1){
		...
		//-------------------------------------------------------------------------------------7-4-2022--UPDATE ROOM CHARGE---
		if(isset($_REQUEST['roomcharge']))
		  { ?>
		  <form method="get" action="script_update_roomcharge.php">
		  <div class="col-xs-4"><br/>
		  <div class="w3-left w3-xlarge">Update Room Charge</div><br/><br/>
		   
		  <?php if(isset($_REQUEST['success'])){ ?><div style="color:#0066FF" class="w3-left w3-large">Update Room Charge Success!</div><br/><?php } ?>
		  
		   <input name="level" type="hidden" value="<?php echo $_REQUEST['level'];?>">
		   <input name="settings" type="hidden" value="<?php echo $_REQUEST['settings'];?>">
		   <input name="roomcharge" type="hidden" value="<?php echo $_REQUEST['roomcharge'];?>">
		   <select class='form-control' name="room_number">
			<?php
			foreach (range(1, 25) as $number) 
			{
			echo "<option value=".$number.">ROOM &nbsp;". $number."</option>";
			} ?>

		   </select>
		   <input required class="form-control" placeholder="New Room Charge" name="new_roomcharge" type="number" step="any" /><br/>
		   <input name="update_roomcharge" type="submit" class="btn btn-primary" value="Update Room Charge" onclick="return confirm('Are you sure?')">
		  </form><br/><br/>
		  
		  <?php
		  $ss="select room_no, room_rate from room";
		  $qq=mysql_query($ss) or die(mysql_error);
		  $rr=mysql_fetch_assoc($qq);
		  echo "<table class='table' cellspacing='1' cellpadding='1'>
					<tr>
						<td>ROOM NO</td><td>ROOM RATE</td>";
		  do{
			  echo "<tr><td>ROOM NO: <b>".$rr['room_no']."</b></td><td>Php <b>".number_format($rr['room_rate'],2)."</b></td></tr>";
	...
	<?php
		  if(isset($_REQUEST['roomovercharge']))
		  { ?>
		  <form method="get" action="script_update_roomovercharge.php">
		  <div class="col-xs-4"><br/>
		  <div class="w3-left w3-xlarge">Update Room Over Charge Rate</div><br/><br/>
		   
		  <?php if(isset($_REQUEST['success'])){ ?><div style="color:#0066FF" class="w3-left w3-large">Update Over Rate Charge Success!</div><br/><?php } ?>
		  
		   <input name="level" type="hidden" value="<?php echo $_REQUEST['level'];?>">
		   <input name="settings" type="hidden" value="<?php echo $_REQUEST['settings'];?>">
		   <input name="roomovercharge" type="hidden" value="<?php echo $_REQUEST['roomovercharge'];?>">
		   <input required class="form-control" placeholder="New Room Charge" name="new_roomovercharge" type="number" step="any" /><br/>
		   <input name="update_roomovercharge" type="submit" class="btn btn-primary" value="Update Room Over Charge" onclick="return confirm('Are you sure?')">
		  </form><br/><br/>
		  ...
		  $sss="select * from room_over_charge_rate";
				$qqq=mysql_query($sss) or die(mysql_error());
				$rrr=mysql_fetch_assoc($qqq);
		  echo $rrr['over_rate']." /minute <br/>";
		  echo "Rate per Hour: Php ".$rrr['over_rate']*60;
		  }
